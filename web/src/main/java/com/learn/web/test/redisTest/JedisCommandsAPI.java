 ...
 */
public interface JedisCommandsAPI {
     ...
     * @param key
     * @param milliseconds
     * @return 返回值同 {expire}一样。
     */
    Long pexpire(String key, long milliseconds);

    /**
     * 与{expire}不一样，expireAt设置的时间不是能存活多久，而是固定的UNIX时间（从1970年开始算起），单位为秒。
     *
     * @param key
     * @param unixTime
     * @return
    ...
    /**
     * 参考 {set(String key, String value, String nxxx, String expx, long
     * time)}
     *
     * @param key
     * @param value
     ...
     * 命令阻塞。 当给定多个 key 参数时，按参数 key 的先后顺序依次检查各个列表，弹出第一个非空列表的头元素。 {http
     ...
     * 基本是完全一样的，除了它们一个是从尾部弹出元素，而另一个是从头部弹出元素。 {http
